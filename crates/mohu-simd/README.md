...
let v = unsafe { \_mm256\_loadu\_ps(ptr) };
...
\### Rule 2 — Every AVX2 function needs `#\[target\_feature(enable = "avx2")]`
...
unsafe fn add\_f32\_avx2(a: \*const f32, b: \*const f32, out: \*mut f32, len: usize) {

&#x20;   // \_mm256\_\* intrinsics are safe to call inside this function
...
\- \*\*AVX-512\*\* functions must use `#\[target\_feature(enable = "avx512f")]`.
...
\- \*\*NEON\*\* functions on aarch64 do \*\*not\*\* need `#\[target\_feature]` because
...
> Calling a `#\[target\_feature]`-gated function on a CPU that does not support
