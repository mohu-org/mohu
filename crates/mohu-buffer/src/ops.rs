...
fn cast_typed<S: Scalar + Send + Sync, D: Scalar + Send + Sync>(
    ...
    if src.is_c_contiguous() && dst.is_c_contiguous() {
        ...
        // Under Miri, rayon's crossbeam-epoch dependency triggers Stacked
        // Borrows violations on slices derived from raw pointers.
        #[cfg(not(miri))]
        src_slice
            .par_iter()
...
/// larger than a few MiB where data will not be immediately re-read.
