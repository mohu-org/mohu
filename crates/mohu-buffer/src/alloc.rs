...
/// # Safety
///
/// `ptr` must be valid for writes of `count` contiguous `f32` values, 32-byte
/// aligned, and `count` must be a multiple of 8.
#[cfg(target_arch = "x86_64")]
...
/// # Safety
///
/// `ptr` must be valid for writes of `count` contiguous `f64` values, 32-byte
/// aligned, and `count` must be a multiple of 4.
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx2")]
