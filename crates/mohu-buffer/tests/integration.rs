...
#[test]
fn full_fills_bytes() {
    // full() takes raw bytes — pass f32 3.5 as bytes
    let fill: f32 = 3.5;
...
#[test]
fn div_scalar_inplace_panics_on_integer_division_by_zero() {
    let mut buf = Buffer::ones(DType::I32, &[10]).unwrap();
    let result = ops::div_scalar_inplace::<i32>(&mut buf, 0);
    assert!(result.is_err());
    assert!(matches!(
        result.unwrap_err(),
        mohu_error::MohuError::DivisionByZero
    ));
}
